...
def resource_path(relative_path):
    ...
    try:
        ...
        # Se não estiver usando PyInstaller, usa o caminho do diretório atual
...
class Calculadora:
    def __init__(self):
        ...
        self.janela.geometry('300x470+700+70')  # Define o tamanho da janela
        self.janela.title("Calculadora SENAI")  # Define o título da janela

        # Definição de cores e fontes
        self.fonte = ("Roboto", 18)  # Fonte usada na interface
        self.botoes_cor = 'secondary'  # Cor dos botões numéricos e de ponto
        self.botoes_operadores_cor = 'danger'  # Cor dos botões de operadores
        self.botoes_especiais_cor = 'info'  # Cor de botões especiais como C

        # Ícone da janela
        icon_path = resource_path('icone.ico')  # Obtém o caminho do ícone
        self.janela.iconbitmap(default=icon_path)  # Define o ícone da janela
        ...
        for i, linha in enumerate(self.botoes):  # Itera sobre as linhas de botões
            for j, texto in enumerate(linha):  # Itera sobre os botões em cada linha
                estilo = self.botoes_operadores_cor if texto in ['C', '←', '%', '/', '*', '-', '+'] else 'secondary.TButton'
                botao = ttk.Button(
                    self.frame_botoes,
                    text=texto,
                    width=6,
        ...
        imagem = imagem.resize((100, 100), Image.LANCZOS)  # Redimensiona a imagem mantendo a qualidade
        ...
        label_imagem.image = imagem  # Armazena uma referência para a imagem (necessário para evitar que a imagem seja destruída pelo garbage collector)
        ...
        self.label_tema.pack(side="top", pady=5)

        # Lista de temas (ComboBox)
        self.temas = ['darkly', 'flatly', 'journal', 'litera', 'lumen', 'minty', 'pulse', 'sandstone', 'united', 'yeti', 'morph', 'simplex', 'superhero']
        ...
        self.seletor_tema.pack(side="left", padx=5)
    ...
    def interpretar_botao(self, valor):
        ...
        if valor == 'C':
            ...
            self.calcular()  # Chama a função de cálculo
        elif valor in '+-*/%':
    ...
    def calcular(self):
        ...
        try:
            resultado = str(eval(expressao))  # Avalia a expressão e exibe o resultado
            self.display.configure(text=resultado)
        except:
            self.display.configure(text="Erro")  # Exibe mensagem de erro caso a avaliação falhe
